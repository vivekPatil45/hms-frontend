        ...
        <div class="bg-card rounded-xl border border-border p-8 shadow-md">
            ...
            <div class="text-left space-y-4 bg-muted/30 p-6 rounded-lg mb-8">
                <div class="flex justify-between">
                    ...
                    <span class="font-medium text-foreground">{{ reservation.reservationId }}</span>
                </div>
                 <div class="flex justify-between">
                    <span class="text-sm text-muted-foreground">Transaction ID</span>
                    <span class="font-medium text-foreground">{{ reservation.transactionId }}</span>
                </div>
                 <div class="flex justify-between">
                 ...
                 <div class="flex justify-between border-t border-border pt-4 mt-2">
                    ...
                    <span class="font-medium text-foreground">Room {{ reservation.room.roomNumber }} ({{ reservation.room.roomType }})</span>
                </div>
                 <div class="flex justify-between">
                    <span class="text-sm text-muted-foreground">Dates</span>
                    <span class="font-medium text-foreground">{{ reservation.checkInDate }} to {{ reservation.checkOutDate }}</span>
                </div>
            </div>
...
export class BookingSuccessComponent implements OnInit {
    ...
    downloadReceipt() {
        if (this.reservation) {
            this.invoiceService.generateInvoice(this.reservation);
        }
    }
