...
import { CustomerService, ComplaintRequest } from '../../../core/services/customer.service';
...
export class CustomerComplaintsComponent implements OnInit {
  ...
  loadComplaints() {
    ...
    this.customerService.getMyComplaints().subscribe({
      next: (response) => {
  ...
  confirmResolution() {
    ...
    this.customerService.updateComplaintStatus(this.selectedComplaint.complaintId, 'CLOSED').subscribe({
      next: (response) => {
  ...
  reopenComplaint() {
    ...
    this.customerService.updateComplaintStatus(this.selectedComplaint.complaintId, 'OPEN').subscribe({
      next: (response) => {
        this.isUpdatingStatus = false;
        if (response.success && response.data) {
          // Update the complaint in the list
