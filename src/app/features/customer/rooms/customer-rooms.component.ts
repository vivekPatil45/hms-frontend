            ...
            <div class="bg-card rounded-xl border border-border overflow-hidden card-hover group">
              ...
              <div class="h-48 bg-muted flex items-center justify-center relative group-hover:scale-105 transition-transform duration-300">
                <div class="text-center text-muted-foreground">
                  ...
                  <p class="text-sm font-medium">Room {{ room.number }}</p>
                </div>
              ...
              <div class="p-5 relative bg-card">
                <div class="mb-4">
                  <div class="flex justify-between items-start mb-2">
                    <div>
                      <h3 class="font-semibold text-lg text-foreground">{{ room.type }} Room</h3>
                      <p class="text-xs text-muted-foreground">Floor {{ room.floor }} • Room {{ room.number }}</p>
                    </div>
...
export class CustomerRoomsComponent {
    ...
    get filteredRooms() {
        return this.rooms.filter(room => {
            // Filter by type
            if (this.searchFilters.type !== 'ALL' && room.type !== this.searchFilters.type) {
                return false;
            }
            // Filter by guest capacity
            if (room.capacity < this.searchFilters.guests) {
                return false;
            }
            return true;
        });
